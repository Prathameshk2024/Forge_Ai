...
import { buildMentorUserPrompt, buildProjectDigest, MENTOR_SYSTEM_PROMPT, parseMentorResponse, } from "./mentor.js";
    ...
    for (const model of [PRIMARY_MODEL, FALLBACK_MODEL]) {
        ...
        for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
            try {
                ...
                if (delay === undefined)
                    break;
                console.warn(`[gemini] ${model} returned ${error.status}, retrying in ${delay}ms`);
                await sleep(delay);
            }
        }
...
app.post("/mentor", async (req, res) => {
    try {
        const { prompt, projectName, files } = req.body;
        ...
        const response = await generateWithRetry({
            ...
            contents: buildMentorUserPrompt(prompt ?? "", projectName ?? "Generated project", digest),
        });
        let explanation;
        try {
