...
function score(path: string): number {
  ...
  if (/(^|\/)(src\/)?(main|index|app)\.(tsx?|jsx?)$/i.test(path)) return 1;
  if (/(^|\/)src\/(pages|routes|views)\//i.test(path)) return 2;
  if (/(^|\/)src\/components\//i.test(path)) return 3;
  if (/(^|\/)src\//i.test(path)) return 4;
  if (/\.(tsx?|jsx?)$/i.test(path)) return 5;
  if (/\.(css|html|json)$/i.test(path)) return 6;
  return 7;
}

/** Selects and truncates project files into the block embedded in the prompt. */
export function buildProjectDigest(files: MentorFile[]): { digest: string; includedPaths: string[] } {
...
export function buildMentorUserPrompt(prompt: string, projectName: string, digest: string): string {
...
export function parseMentorResponse(raw: string): unknown {
  const cleaned = raw
    .trim()
    .replace(/^```(?:json)?\s*/i, '')
    .replace(/```$/, '')
